...
.PARAMETER TargetEnv
    Entorno destino: pre, pro o ambos.
...
if ($ShowHelp) {
    ...
    Write-Host "      .\pacdep.ps1 -TargetEnv ambos" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "    -TargetEnv <pre|pro|ambos>    Entorno destino"
...
if (-not $ExportOnly -and -not $TargetEnv) {
    ...
    Write-Log "   Uso: .\pacdep.ps1 -TargetEnv <pre|pro|ambos>"
    Write-Log "   Uso: .\pacdep.ps1 -ExportOnly"
    Write-Log "   Uso: .\pacdep.ps1 -ImportOnly -TargetEnv <pre|pro|ambos>"
    exit 1
}
...
# Booleans de destino (evita repetir el patron TargetEnv -eq "pre" -or "ambos")
$deployPre = $TargetEnv -eq "pre" -or $TargetEnv -eq "ambos"
$deployPro = $TargetEnv -eq "pro" -or $TargetEnv -eq "ambos"
...
if ($deployPro) {
    ...
    if ($confirmPro -ne "S" -and $confirmPro -ne "s") {
        ...
        if ($TargetEnv -eq "ambos") {
            Write-Log "  (Tambien se cancelo PRE al estar en modo 'ambos')."
        }
        exit 0
    }
}
