...
.PARAMETER UnpackOnly
    Solo desempaquetar solution.zip en la carpeta solution.
...
.EXAMPLE
    .\pacdep.ps1 -UnpackOnly
    Desempaqueta solution.zip en la carpeta solution.
...
if (-not $ShowHelp -and -not $TargetEnv -and -not $ExportOnly -and -not $ImportOnly -and -not $UnpackOnly) {
    $ShowHelp = $true
}
...
if ($ShowHelp) {
    ...
    Write-Host ""
    Write-Host "    Solo desempaquetar solution.zip en carpeta solution/:"
    Write-Host "      .\pacdep.ps1 -UnpackOnly" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host "    -UnpackOnly                   Solo desempaquetar solution.zip"
...
$solutionUnpackDir = Join-Path $PSScriptRoot "solution"
...
if ($UnpackOnly -and ($ExportOnly -or $ImportOnly)) {
    Write-Log "ERROR: -UnpackOnly es incompatible con -ExportOnly y -ImportOnly."
    Write-Log "   Uso: .\pacdep.ps1 -UnpackOnly"
    exit 1
}

if (-not $ExportOnly -and -not $TargetEnv -and -not $UnpackOnly) {
...
if ($UnpackOnly -and $TargetEnv) {
    Write-Log "AVISO: -TargetEnv no tiene efecto con -UnpackOnly."
}

if ($UnpackOnly -and $SkipVersionIncrement) {
    Write-Log "AVISO: -SkipVersionIncrement no tiene efecto con -UnpackOnly."
}
...
if ($UnpackOnly) { Write-Log "  Modo:     Solo unpack" }
if ($SkipVersionIncrement -and -not $ImportOnly -and -not $UnpackOnly) { Write-Log "  Version:  Sin incrementar" }
Write-Log ""
...
# VALIDAR PAC CLI
# ─────────────────────────────────────────────────────────────────────────────
Write-Log "[1/9] Verificando Power Platform CLI (pac)..."
...
# ─────────────────────────────────────────────────────────────────────────────
if ($UnpackOnly) {
    Write-Log "[2/9] Desempaquetando solucion (modo -UnpackOnly)..."

    if (-not (Test-Path $unmanagedZip)) {
        Write-Log "  ERROR: No se encontro solution.zip para desempaquetar."
        Write-Log "  Ejecuta primero una exportacion: .\pacdep.ps1 -ExportOnly"
        exit 1
    }

    if (Test-Path $solutionUnpackDir) {
        Write-Log "  -> Limpiando carpeta destino existente..."
        Remove-Item $solutionUnpackDir -Recurse -Force
    }
    New-Item -ItemType Directory -Force -Path $solutionUnpackDir | Out-Null

    Write-Log "  -> Ejecutando pac solution unpack..."
    pac solution unpack --zipfile "$unmanagedZip" --folder "$solutionUnpackDir" --packagetype Unmanaged
    Assert-PacSuccess "Desempaquetar solution.zip"

    $itemCount = @(Get-ChildItem -Path $solutionUnpackDir -Recurse -Force -ErrorAction SilentlyContinue).Count
    Write-Log "  OK: Solucion desempaquetada en: $solutionUnpackDir"
    Write-Log "  Items generados: $itemCount"
    Write-Log ""

    $stopwatch.Stop()
    $elapsed = $stopwatch.Elapsed.ToString("mm\:ss")
    Write-Log "==============================================="
    Write-Log "  UNPACK COMPLETADO (-UnpackOnly)"
    Write-Log "  Carpeta:  $solutionUnpackDir"
    Write-Log "  Duracion: $elapsed"
    Write-Log "  Log:      $logFile"
    Write-Log "==============================================="
    Send-Notification -Message "Unpack completado: $elapsed"
    exit 0
}

Write-Log "[2/9] Omitido (solo aplica con -UnpackOnly)"
Write-Log ""

# ─────────────────────────────────────────────────────────────────────────────
# VALIDAR / GENERAR config.json
# ─────────────────────────────────────────────────────────────────────────────
Write-Log "[3/9] Validando config.json..."
...
# ─────────────────────────────────────────────────────────────────────────────
Write-Log "[4/9] Validando perfiles de autenticacion..."
if (-not $ImportOnly) {
...
# ─────────────────────────────────────────────────────────────────────────────
if (-not $ImportOnly) {
    Write-Log "[5/9] Limpiando exportaciones anteriores..."
    ...
    Write-Log "[5/9] Validando zips existentes para importacion..."
...
if (-not $ImportOnly) {
    ...
    if ($SkipVersionIncrement) {
        Write-Log "[6/9] Omitiendo incremento de version (-SkipVersionIncrement)..."
        Write-Log "  Version actual en DEV: $currentVersion"
        $newVersion = $currentVersion
    }
    else {
        Write-Log "[6/9] Incrementando version de la solucion en DEV..."
    ...
    Write-Log "[7/9] Exportando solucion desde DEV..."
    ...
    Write-Log "[6/9] Omitido (modo -ImportOnly)"
    Write-Log "[7/9] Omitido (modo -ImportOnly)"
    $newVersion = $null
    Write-Log ""
}
...
Write-Log "[8/9] Verificando settings (variables de entorno / conexiones)..."
...
# ─────────────────────────────────────────────────────────────────────────────
Write-Log "[9/9] Importando solucion..."
